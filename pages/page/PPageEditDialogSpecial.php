...
(function($){
    $(document).ready(function() {
        ...
        // Datepicker
        $('input#dateFromPED').datepicker({
            minDate: 0,
            dateFormat: "yy-mm-dd"
        });
        $('input#dateTomPED').datepicker();
...
<div id="dialogPageTextChange" title="Ändra text">
    <form id="dialogPageTextChangeForm" action='?p=page-save' method='POST'>
        ...
        <h3>Datum</h3>
        <input id='dateFromPED' type='{$type}' name='dateFrom' value='{$dateFrom}'> - 
        <input id='dateTomPED' type='{$type}' name='dateTom' value='{$dateTom}'>
